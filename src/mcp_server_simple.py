...
All variable data goes in attrs, only 2 status values (active/archived).
"""
...
def upsert(
    ...
    Same key = update existing item. Variable fields go in attrs.
    ...
    Args:
        ...
        attrs: Optional dict/JSON for variable data (priority, tags, due_date, parent_key, etc.)
    ...
    Examples:
        ...
        upsert(
            kind='knowledge',
            key='knee-health',
            content='Knee tracking: keep knees over toes...',
            attrs={'tags': ['injury-prevention', 'squat-form']}
        )
    """
    ...
    if attrs is not None and isinstance(attrs, str):
        try:
            ...
            attrs_value = {'raw_attrs': attrs}

    with get_session() as session:
        # The CRUD function handles moving legacy fields to attrs
...
def log(
    ...
    Args:
        ...
        attrs: Optional dict/JSON for structured data (exercises, weight, etc.)
    ...
    Examples:
        ...
        log(
            kind='workout',
            content='Squats 3x5 @ 225lbs',
            occurred_at='2025-01-15T10:00:00Z',
            attrs={'exercises': [{'name': 'Squat', 'sets': 3, 'reps': 5, 'weight': 225}]}
        )

        # Update existing event
        log(
            event_id='abc123...',
            content='Squats 3x5 @ 230lbs',  # Corrected weight
            attrs={'exercises': [{'name': 'Squat', 'sets': 3, 'reps': 5, 'weight': 230}]}
        )
    """
    user_id = _get_user_id()
    ...
    with get_session() as session:
        if event_id:
            ...
            return crud.update_event(
                ...
                attrs=attrs_value,
                replace_attrs=False  # Merge by default
            )
        else:
            # Create new event
