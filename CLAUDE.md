...
The `planning/` folder provides a **multi-agent validation system** for creating high-quality training plans with systematic safety checks.

**Structure:**
- `commands/` - Workflows for program/week/workout creation (create-program.md, plan-week.md, etc.)
- `protocols/` - 8+ evidence-based protocols (knee-health, vo2max-development, concurrent-training, recovery-deload, etc.)
- `.claude/agents/plan-validator.md` - Validation agent that cross-checks plans against protocols and user constraints

**6-Step Workflow:**
1. Extract context from MCP (`overview`, `get`)
2. Reference relevant protocols
3. Design strategy (draft program/week/workout)
4. Validate with plan-validator agent
5. Propose to user
6. Save on approval

**When to Use:**
- **Planning System**: Creating programs, need validation gates, safety-critical plans, framework-agnostic
- **Skills Folder**: Real-time coaching, quick questions, Claude Code only

Both use the same MCP server for data persistence. See [planning/README.md](planning/README.md) and [protocols/INDEX.md](planning/protocols/INDEX.md) for details.
